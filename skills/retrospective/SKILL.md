...
<!-- Source: https://github.com/xxthunder/xxthunder-dev-skills/tree/develop/skills/retrospective -->
...
4. **Guidelines consulted**: Check if relevant guidelines exist in project documentation (e.g., `AGENTS.md`, `CLAUDE.md`, development principles, or equivalent)
...
- Example: "I didn't run tests before committing despite project guidelines requiring it"
...
- Example: "Guidelines say 'use appropriate error handling' but don't specify what that means in this context"
...
Based on the root cause, identify the appropriate file to update. Look for:
- **Agent instructions** (`AGENTS.md`, `CLAUDE.md`) - for agent-specific workflows, tool usage, project-specific patterns
- **Development principles** (`docs/development-principles.md`, `CONTRIBUTING.md`) - for general development principles
- **Other project guidelines** - adapt to whatever the project uses
...
1. **Show the proposed change**:
   ...
   - Explain which file will be updated and why
...
Looking at the recent commit, I added a new function but only wrote tests
after you prompted me.

Checking project guidelines... I see "All code must include tests" but I
violated this by committing code without tests first.

Root cause: Guideline violation (I didn't follow TDD)

I should strengthen the TDD guideline to emphasize NEVER committing code
without tests. Should I add:
...
### Example 2: Wrong Tool Selection

**User**: "Why did you use Grep directly instead of the Task tool?"

**Claude Response**:
```
You're right to question this. Let me review the guidance:

The guidelines say "use Task tool with subagent_type=Explore for open-ended
searches" but you asked "find files handling authentication" which felt
specific enough for Grep.

Root cause: Unclear guideline (boundary between "specific" and "open-ended"
search is ambiguous)

I should clarify the guideline with concrete examples:
...
- Need to connect information across files
...
**When in doubt**: Use Task tool - better to over-explore than miss context."

Should I add this clarification?
```
...
**Update agent instructions** (e.g., `AGENTS.md`, `CLAUDE.md`) for:
- Project-specific workflows
- Tool usage patterns (Task, Grep, Edit, etc.)
- Environment-specific behavior (CI vs. interactive)
- Integration patterns (package managers, launchers, etc.)
- Commit and PR creation workflows

**Update development principles** (e.g., `docs/development-principles.md`, `CONTRIBUTING.md`) for:
...
**Update both** when:
- The principle is general (development principles) but needs project-specific application (agent instructions)
